	...
	try {
		...
		if (m.key.remoteJid === 'status@broadcast') {
			...
			setTimeout(() => {
				...
				console.log((/protocolMessage/i.test(mt)) ? `💀 Telah Menghapus Story Nya User ➡️ ${m.key.participant.split('@')[0]}` : '🥶 Telah Melihat Story Nya User ➡️ '+m.key.participant.split('@')[0]);
			}, cuy.faston);
		}
	} catch (e) {
		console.log('');
